...
public class QueryFactory {
	...
	String databaseURI;
	String sru_server = "https://sru.k10plus.de/";
	String version = "1.2";
	 ...
	 * 
	 * @param databaseURI a database URI from http://uri.k10plus.de/database/
	 */
	 ...
	 * 
	 * @param searchString CQL style search string
	 *                     https://www.loc.gov/standards/sru/cql/
	 * @param max_results  the max_results parameter of the query, should not exceed
	 *                     1000
	...
	public URL getQueryCQL(String searchString, int max_results, int pos) {
		try {
			return new URL(sru_server + databaseURI + "?version=" + version + "&operation=" + operation + "&query="
					+ searchString.replaceAll(" ", "+") + "&recordSchema=" + recordSchema + "&maximumRecords="
					+ max_results + "&startRecord=" + pos);
