...
public class CSVWriter extends AbstractCSVWriter{
	 ...
	 */
	public CSVWriter(String destination, DataFields[] datafields, String sep, Predicate<Title> predicateFunction, boolean superInfos) {
	...
	public void addTitle(Title title) {
		...
		for (Copy c : title.copies) {
			if (c.epn=="") continue; //if copy numbers are not in sequence
			String s = "";
			for (DataFields df : datafields) {
				...
				case AUTHOR_OTHER:
					...
					break;
				case AUTHORS:
					s+=title.author.replaceAll(quot, quot_replacement)+" "+title.co_authors.replaceAll(quot, quot_replacement);
					break;
				case YEAR:
				...
				case LOCAL_SYSTEMATIC:
					...
					break;
				case FR:
					s+=title.getFR().replaceAll(quot, quot_replacement);
					break;
				case ON_LOAN:
